...
final class APIService {
    ...
    func translate(text: String, language: String) async -> Result<GrammarCheckResponse, APIError> {
        guard let url = URL(string: "https://kb-api.minailabs.io/translate") else {
            return .failure(.invalidURL)
        }
        ...
        do {
            request.httpBody = try JSONEncoder().encode(TranslateRequest(text: text, language: language))
        } catch {
            return .failure(.requestFailed(error))
        }
    ...
    func paraphrase(text: String) async -> Result<GrammarCheckResponse, APIError> {
        guard let url = URL(string: "https://kb-api.minailabs.io/paraphrase") else {
            return .failure(.invalidURL)
        }
        ...
        do {
            request.httpBody = try JSONEncoder().encode(ParaphraseRequest(text: text))
        } catch {
            return .failure(.requestFailed(error))
        }
    ...
    func reply(text: String) async -> Result<GrammarCheckResponse, APIError> {
        guard let url = URL(string: "https://kb-api.minailabs.io/reply") else {
            return .failure(.invalidURL)
        }
